...
def extract_environment_from_image_config(config):
    ...
    for env_var in env_list:
        ...
        if '\n' in env_var:
            ...
            print(f'Skipping environment variable `{env_var.split("=", 1)[0]}`: its value contains newlines.')
            continue
        escaped_env_var = env_var.translate(str.maketrans({'\\': r'\\', '"': r'\"'}))
        env_var_name = escaped_env_var.split('=', 1)[0]
        if env_var_name == f'PATH' or env_var_name == f'LD_LIBRARY_PATH':
            ...
            continue
        env_var_value = escaped_env_var.split('=', 1)[1]
