...
def generate_compile_dockerfile(image, env):
    ...
    with open(dockerfile_path, 'w') as dockerfile:
        dockerfile.write(env.get_template(
            f'Dockerfile.{env.globals["Distro"]}.build.template').render())
...
def gsc_build_graphene(args):
    ...
    build_docker_image(image, image, 'Dockerfile.build',
                       rm=args.rm, nocache=args.no_cache, buildargs=buildargs_dict)
    ...
    print(f'Successfully build Graphene Docker image {image}')
...
# Sign Docker image which was previously build via `gsc build`.
