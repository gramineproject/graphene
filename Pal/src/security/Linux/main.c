...
static void do_bootstrap(void* args, int* pargc, const char*** pargv, const char*** penvp,
                         ElfW(auxv_t) * *pauxv, void** baseaddr, const char** program_name) {
    ...
    for (; *(auxv - 1); auxv++)
        ;
    ElfW(auxv_t) * av;
    for (av = (ElfW(auxv_t)*)auxv; av->a_type != AT_NULL; av++) switch (av->a_type) {
            case AT_PAGESZ:
...
int open_manifest(const char** argv) {
    ...
    memcpy((void*)manifest_name + len, ".manifest", static_strlen(".manifest"));
...
static int do_relocate(ElfW(Dyn) * dyn, ElfW(Addr) addr) {
    ...
    for (; r < end; r++) *(ElfW(Addr)*)(addr + r->r_offset) = addr + r->r_addend;

    return 0;
}

static void* find_symbol(const ElfW(Dyn) * dyn, ElfW(Addr) addr, const char* name) {
...
static int load_static(const char* filename, void** load_addr, void** entry, ElfW(Dyn) * *dyn,
    ...
    *phnum = header->e_phnum;
    ...
    for (ph = phdr; ph < &phdr[header->e_phnum]; ph++) switch (ph->p_type) {
            case PT_DYNAMIC:
                *dyn = (void*)ph->p_vaddr;
                break;
    ...
    base = INLINE_SYSCALL(mmap, 6, NULL, maplength, c->prot, MAP_PRIVATE | MAP_FILE, fd, c->mapoff);
    ...
    for (; c < &loadcmds[nloadcmds]; c++) {
        ElfW(Addr) addr =
            INLINE_SYSCALL(mmap, 6, base + c->mapstart, c->mapend - c->mapstart, c->prot,
                           MAP_PRIVATE | MAP_FILE | MAP_FIXED, fd, c->mapoff);
        if (IS_ERR_P(addr)) {
            ret = -ERRNO_P(addr);
            goto out;
        }
...
extern __typeof(___dl_debug_state) _dl_debug_state __attribute((alias("___dl_debug_state")));
...
struct r_debug ___r_debug = {1, NULL, (ElfW(Addr)) & ___dl_debug_state, RT_CONSISTENT, 0};

extern __typeof(___r_debug) _r_debug __attribute((alias("___r_debug")));

int ioctl_set_graphene(struct config_store* sandbox_config, int npolices,
                       const struct graphene_user_policy* policies);

int set_sandbox(struct config_store* sandbox_config, struct pal_sec* pal_sec_addr, void* pal_addr) {
    struct graphene_user_policy policies[] = {
        ...
        {
            .type = GRAPHENE_FS_PATH | GRAPHENE_FS_READ, .value = "/proc/meminfo",
        },
    };

    return ioctl_set_graphene(sandbox_config, sizeof(policies) / sizeof(policies[0]), policies);
}
...
void do_main(void* args) {
    ...
    ret = load_static(PAL_LOADER, &pal_addr, &pal_entry, &pal_dyn, &pal_phoff, &pal_phnum);
    ...
    struct pal_sec* pal_sec_addr = find_symbol(pal_dyn, (ElfW(Addr))pal_addr, "pal_sec");
    if (!pal_sec_addr) {
    ...
    void* stack = __alloca(sizeof(unsigned long) + sizeof(char*) * (argc + 2) +
                           sizeof(char*) * envc + sizeof(ElfW(auxv_t)) * auxc);
    ...
    *(unsigned long*)stack = argc + 1;
    ...
    for (ElfW(auxv_t)* av = new_auxv; av->a_type != AT_NULL; av++) switch (av->a_type) {
            case AT_ENTRY:
