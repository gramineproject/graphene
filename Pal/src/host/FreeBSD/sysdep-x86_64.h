...
/* BSD useds SYS_* sys call names, so we can use these to map syscall names to syscall numbers from
 * syscall.h */
...
#define ENTRY(name)                                        \
    ...
    ASM_TYPE_DIRECTIVE(name, @function).align ALIGNARG(4); \
    name:                                                  \
    cfi_startproc;
...
#define DO_CALL(syscall_name, args)                   \
    DOARGS_##args movl $SYS_ify(syscall_name), % eax; \
    syscall;
...
#define INTERNAL_SYSCALL_NCS(name, err, nr, args...)                                               \
    ({                                                                                             \
        LOAD_ARGS_##nr(args) LOAD_REGS_##nr unsigned long resultvar;                               \
        asm volatile("int $0x80\n\t" : "=a"(resultvar) : "0"(name)ASM_ARGS_##nr : "memory", "cc"); \
...
To simplify matters and reuse a lot of the Linux code, we change rax to negative after checking the
carry flag.
*/
