...
#else
        int ret = INLINE_SYSCALL(sigaction, 4, sigs[i], &action, NULL, sizeof(sigset_t));
#endif
            if (IS_ERR(ret)) return -PAL_ERROR_DENIED;
...
#else
        ret     = INLINE_SYSCALL(sigprocmask, 4, SIG_UNBLOCK, &mask, NULL, sizeof(sigset_t));
#endif
    }
...
#else
        ret     = INLINE_SYSCALL(sigprocmask, 4, SIG_BLOCK, &mask, NULL, sizeof(sigset_t));
#endif
    }
...
#else
        int ret = INLINE_SYSCALL(sigaction, 4, sigs[i], SIG_DFL, NULL, sizeof(_sigset_t));
#endif
            if (IS_ERR(ret)) return -PAL_ERROR_DENIED;
    }
    return 0;
...
#define DECLARE_HANDLER_HEAD(event)                            \
    static struct exception_handler handler_##event = {        \
        .lock = MUTEX_HANDLE_INIT, .upcall = NULL, .flags = 0, \
    };
...
static void _DkGenericEventTrigger(int event_num, PAL_UPCALL upcall, int flags, PAL_NUM arg,
                                   struct pal_frame* frame, ucontext_t* uc, void* eframe) {
...
static bool _DkGenericSignalHandle(int event_num, siginfo_t* info, struct pal_frame* frame,
    ...
    if (upcall) {
        ...
        if (event_num == PAL_EVENT_ARITHMETIC_ERROR || event_num == PAL_EVENT_MEMFAULT ||
            event_num == PAL_EVENT_ILLEGAL)
            arg = (PAL_NUM)(info ? info->si_addr : 0);

        _DkGenericEventTrigger(event_num, upcall, flags, arg, frame, uc, eframe);
        return true;
    }
...
static struct pal_frame* get_frame(ucontext_t* uc) {
    ...
    for (unsigned long ptr = rbp - sizeof(unsigned long); ptr > last_rbp; ptr -= 8) {
...
static void _DkGenericSighandler(int signum, siginfo_t* info, ucontext_t* uc) {
    ...
    if (frame && frame->func != &_DkGenericSighandler && signum != SIGCONT && signum != SIGINT &&
        signum != SIGTERM) {
...
void _DkRaiseFailure(int error) {
    ...
    if (upcall)
        _DkGenericEventTrigger(PAL_EVENT_FAILURE, upcall, flags, error, NULL, NULL, NULL);
}
...
struct signal_ops on_signals[PAL_EVENT_NUM_BOUND] = {
    /* DivZero     */ {.signum = {SIGFPE, 0}, .handler = _DkGenericSighandler},
    /* MemFault    */ {.signum = {SIGSEGV, SIGBUS, 0}, .handler = _DkGenericSighandler},
    /* Illegal     */ {.signum = {SIGILL, 0}, .handler = _DkGenericSighandler},
    /* Quit        */ {.signum = {SIGTERM, 0, 0}, .handler = _DkTerminateSighandler},
    /* Suspend     */ {.signum = {SIGINT, 0}, .handler = _DkTerminateSighandler},
    /* Resume      */ {.signum = {SIGCONT, 0}, .handler = _DkGenericSighandler},
};
...
static int _DkPersistentEventUpcall(int event_num, PAL_UPCALL upcall, int flags) {
    struct exception_handler* handler = pal_handlers[event_num];
...
static int _DkGenericEventUpcall(int event_num, PAL_UPCALL upcall, int flags) {
    int nsigs, *sigs = on_signals[event_num].signum;
    for (nsigs = 0; sigs[nsigs]; nsigs++)
        ;
...
static int _DkDummyEventUpcall(int event_num, PAL_UPCALL upcall, int flags) {
    struct exception_handler* handler = pal_handlers[event_num];
...
static void _DkCompatibilitySighandler(int signum, siginfo_t* info, ucontext_t* uc) {
    /* not implemented */
}
...
#else
    ret         = INLINE_SYSCALL(sigaction, 4, SIGCHLD, &action, NULL, sizeof(sigset_t));
#endif
        if (IS_ERR(ret)) {
