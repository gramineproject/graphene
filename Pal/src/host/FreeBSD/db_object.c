...
static int _DkObjectWaitOne(PAL_HANDLE handle, int timeout) {
    ...
    if (handle->hdr.flags & HAS_FDS) {
        ...
        for (int i = 0; i < MAX_FDS; i++) {
            ...
            if ((handle->hdr.flags & RFD(i)) && !(handle->hdr.flags & ERROR(i)))
                events |= POLLIN;

            if ((handle->hdr.flags & WFD(i)) && !(handle->hdr.flags & WRITABLE(i)) &&
        ...
        int ret = INLINE_SYSCALL(poll, 3, &fds, nfds, timeout ? timeout : -1);
...
int _DkObjectsWaitAny(int count, PAL_HANDLE* handleArray, uint64_t timeout, PAL_HANDLE* polled) {
    ...
    for (i = 0; i < count; i++) {
        ...
        for (j = 0; j < MAX_FDS; j++) {
            ...
            if ((hdl->hdr.flags & RFD(j)) && !(hdl->hdr.flags & ERROR(j)))
                events |= POLLIN;

            if ((hdl->hdr.flags & WFD(j)) && !(hdl->hdr.flags & WRITABLE(j)) &&
    ...
    for (i = 0; i < nfds; i++) {
        ...
        for (j = 0; j < MAX_FDS; j++)
            if ((hdl->hdr.flags & (RFD(j) | WFD(j))) && hdl->hdr.fds[j] == fds[i].fd)
                break;
