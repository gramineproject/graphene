...
static void _DkGenericEventTrigger(PAL_IDX event_num, PAL_EVENT_HANDLER upcall, PAL_NUM arg,
...
static void _DkGenericSighandler(int signum, siginfo_t* info, struct ucontext* uc) {
#if BLOCK_SIGFUALT == 1
...
static void _DkTerminateSighandler(int signum, siginfo_t* info, struct ucontext* uc) {
    struct pal_frame* frame = get_frame(uc);
    void* eframe;
...
static void _DkPipeSighandler(int signum, siginfo_t* info, struct ucontext* uc) {
    return;
}
...
struct signal_ops on_signals[] = {
        [PAL_EVENT_ARITHMETIC_ERROR] = {.signum = {SIGFPE, 0}, .handler = _DkGenericSighandler},
        [PAL_EVENT_MEMFAULT] = {.signum = {SIGSEGV, SIGBUS, 0}, .handler = _DkGenericSighandler},
        [PAL_EVENT_ILLEGAL] = {.signum = {SIGILL, SIGSYS, 0}, .handler = _DkGenericSighandler},
        [PAL_EVENT_QUIT] = {.signum = {SIGTERM, 0, 0}, .handler = _DkTerminateSighandler},
        [PAL_EVENT_SUSPEND] = {.signum = {SIGINT, 0}, .handler = _DkTerminateSighandler},
        [PAL_EVENT_RESUME] = {.signum = {SIGCONT, 0}, .handler = _DkGenericSighandler},
};
...
void signal_setup(void) {
    ...
    int events[] = {
        PAL_EVENT_ARITHMETIC_ERROR, PAL_EVENT_MEMFAULT, PAL_EVENT_ILLEGAL, PAL_EVENT_QUIT,
        PAL_EVENT_SUSPEND,          PAL_EVENT_RESUME,
    };
