...
static int _DkObjectWaitOne(PAL_HANDLE handle, int64_t timeout) {
    ...
    if (HANDLE_HDR(handle)->flags & HAS_FDS) {
        ...
        int ret = INLINE_SYSCALL(ppoll, 5, &fds, nfds, timeout >= 0 ? &timeout_ts : NULL, NULL, 0);
...
int _DkObjectsWaitAny(int count, PAL_HANDLE* handleArray, int64_t timeout, PAL_HANDLE* polled) {
    ...
    ret = INLINE_SYSCALL(ppoll, 5, fds, nfds, timeout >= 0 ? &timeout_ts : NULL, NULL, 0);
    ...
    for (i = 0; i < nfds; i++) {
        ...
        for (j = 0; j < MAX_FDS; j++)
            if ((HANDLE_HDR(hdl)->flags & (RFD(j) | WFD(j))) && hdl->generic.fds[j] == fds[i].fd)
                break;
...
HEAP_ALLOC_RECORD* collect_heap_alloc_records(PAL_NUM max_records) {
    HEAP_ALLOC_RECORD* records = malloc(sizeof(HEAP_ALLOC_RECORD) * max_records);
    ...
    for (; ptr; ptr = ptr->hdr.heap_trace.next) {
        assert(!ptr->hdr.heap_trace.next ||
               ptr->hdr.heap_trace.next->hdr.heap_trace.pprev == &ptr->hdr.heap_trace.next);
