...
static char* str_ptrace_request(enum __ptrace_request request)
{
    ...
    switch (request) {
        ...
        case PTRACE_PEEKTEXT:   return "PEEKTEXT";
        case PTRACE_PEEKDATA:   return "PEEKDATA";
        case PTRACE_POKETEXT:   return "POKETEXT";
        case PTRACE_POKEDATA:   return "POKEDATA";
        case PTRACE_PEEKUSER:   return "PEEKUSER";
        case PTRACE_POKEUSER:   return "POKEUSER";
        case PTRACE_GETREGS:    return "GETREGS";
        case PTRACE_SETREGS:    return "SETREGS";
        case PTRACE_SINGLESTEP: return "SINGLESTEP";
        case PTRACE_CONT:       return "CONT";
        case PTRACE_KILL:       return "KILL";
...
static long int host_ptrace(enum __ptrace_request request, pid_t tid,
                      void * addr, void * data)
{
    ...
    if (!is_dbginfo_addr)
        DEBUG("[GDB %d] Executed host ptrace(%s, %d, %p, %p) = %ld\n",
                getpid(), str_ptrace_request(request), tid, addr, data, ret);
    ...
    if (ret >= 0 && (request == PTRACE_PEEKTEXT ||
                     request == PTRACE_PEEKDATA ||
                     request == PTRACE_PEEKUSER)) {
        ret = res;
    }
...
static void * get_gpr_addr(int memdev, pid_t tid, struct enclave_dbginfo * ei)
{
    ...
    DEBUG("[enclave thread %d] TCS at 0x%lx: TCS.ossa = 0x%lx TCS.cssa = %d\n",
            tid, (uint64_t) tcs_addr, tcs_part.ossa, tcs_part.cssa);
...
static int peek_gpr(int memdev, pid_t tid, struct enclave_dbginfo * ei,
                  sgx_arch_gpr_t * gpr)
{
    int ret;
...
static int poke_gpr(int memdev, pid_t tid, struct enclave_dbginfo * ei,
...
static int peek_user(int memdev, pid_t tid, struct enclave_dbginfo * ei,
                   struct user * ud)
{
    int ret;
    sgx_arch_gpr_t gpr;
...
static int poke_user(int memdev, pid_t tid, struct enclave_dbginfo * ei,
...
static int open_memdevice(pid_t tid, int * memdev, struct enclave_dbginfo ** ei)
{
    ...
    for (int off = 0; off < sizeof(eib); off += sizeof(long int)) {
        ret = host_ptrace(PTRACE_PEEKDATA, tid,
...
long int ptrace(enum __ptrace_request request, ...)
{
    ...
    DEBUG("[GDB %d] Intercepted ptrace(%s, %d, %p, %p)\n",
            getpid(), str_ptrace_request(request), tid, addr, data);
    ...
    switch (request) {
        ...
        case PTRACE_PEEKDATA: {
            ...
            /* Weirdness with fprintf(stderr, ...) is needed to circumvent
             * GDB bug of thread_get_info_callback: cannot get thread info */
            fprintf(stderr, "");
            ret = pread(memdev, &res, sizeof(long int), (unsigned long) addr);
            if (ret < 0) {
                /* Below is dead code (we assume that -2048 never hits) but
                 * required to circumvent GDB bug. The only thing this code
                 * adds to this library is some more assembly, so maybe it
                 * shifts some sections around (so GDB is satisfied with
                 * some offsets in main memory)? */
                if (ret == -2048) {
                    fprintf(stderr, "[GDB %d] pread hits a GDB bug, this is needed for some reason (%d, %d, %p, %p)\n",
                            getpid(), request, tid, addr, data);
                }
            }
        ...
        case PTRACE_POKEDATA: {
            ...
            return pwrite(memdev, &data, sizeof(long int), (off_t) addr);
        }
        ...
        case PTRACE_POKEUSER: {
            ...
            *(long int *)((void *) &userdata + (off_t) addr) = (long int) data;
            return poke_user(memdev, tid, ei, &userdata);
        }
        ...
        case PTRACE_GETREGS: {
            ...
            return peek_regs(memdev, tid, ei, (struct user_regs_struct *) data);
        }
        ...
        case PTRACE_SETREGS: {
            ...
            return poke_regs(memdev, tid, ei, (struct user_regs_struct *) data);
        }
