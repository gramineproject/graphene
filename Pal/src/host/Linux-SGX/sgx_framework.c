...
static int gsgx_device = -1;
static int isgx_device = -1;

static void* zero_pages = NULL;
static size_t zero_pages_size = 0;
                         ...
                         const char * comment)
{
    ...
    if (!zero_pages) {
        ...
        zero_pages = (void*)INLINE_SYSCALL(mmap, 6, NULL, g_page_size, PROT_READ,
                                           MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
        if (IS_ERR_P(zero_pages)) {
            SGX_DBG(DBG_I, "Cannot mmap zero pages %ld\n", ERRNO_P(zero_pages));
            return -ENOMEM;
        }
        zero_pages_size = g_page_size;
    }
...
#ifdef SGX_DCAP_NEW
    if (zero_pages_size < size) {
        /* not enough contigious zero pages to back up enclave pages, allocate more */
        ret = INLINE_SYSCALL(munmap, 2, zero_pages, zero_pages_size);
        if (IS_ERR(ret)) {
            SGX_DBG(DBG_I, "Cannot unmap zero pages %d\n", ret);
            return -ERRNO(ret);
        }

        zero_pages = (void*)INLINE_SYSCALL(mmap, 6, NULL, size, PROT_READ,
                                           MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
        if (IS_ERR_P(zero_pages)) {
            SGX_DBG(DBG_I, "Cannot map zero pages %ld\n", ERRNO_P(zero_pages));
            return -ENOMEM;
        }
        zero_pages_size = size;
    }
    ...
    ret = INLINE_SYSCALL(ioctl, 3, isgx_device, SGX_IOC_ENCLAVE_ADD_PAGES, &param);
    if (IS_ERR(ret)) {
    ...
    while (added_size < size) {
        ret = INLINE_SYSCALL(ioctl, 3, isgx_device, SGX_IOC_ENCLAVE_ADD_PAGE, &param);
        if (IS_ERR(ret)) {
            SGX_DBG(DBG_I, "Enclave EADD returned %d\n", ret);
            return -ERRNO(ret);
        }
                 ...
                 sgx_arch_token_t * token)
{
    ...
    int ret = INLINE_SYSCALL(ioctl, 3, isgx_device, SGX_IOC_ENCLAVE_INIT, &param);
