...
int open_sgx_driver(bool need_gsgx) {
    ...
    g_isgx_device = INLINE_SYSCALL(open, 3, CONFIG_SGX_DRIVER_DEVICE, O_RDWR | O_CLOEXEC, 0);
    if (g_isgx_device < 0) {
...
int read_enclave_token(int token_file, sgx_arch_token_t* token) {
    ...
    log_debug("    attr.flags:            0x%016lx\n", token->body.attributes.flags);
    log_debug("    attr.xfrm:             0x%016lx\n", token->body.attributes.xfrm);
    ...
    log_debug("    LE cpu_svn:            %s\n", ALLOCA_BYTES2HEXSTR(token->cpu_svn_le.svn));
    log_debug("    LE isv_prod_id:        %02x\n", token->isv_prod_id_le);
    log_debug("    LE isv_svn:            %02x\n", token->isv_svn_le);
    log_debug("    LE masked_misc_select: 0x%08x\n",   token->masked_misc_select_le);
    log_debug("    LE attr.flags:         0x%016lx\n", token->attributes_le.flags);
    log_debug("    LE attr.xfrm:          0x%016lx\n", token->attributes_le.xfrm);
#endif
...
int create_enclave(sgx_arch_secs_t* secs, sgx_arch_token_t* token) {
    ...
    if (IS_ERR_P(addr)) {
        ...
        log_error("ECREATE failed in allocating EPC memory (errno = %ld)\n", -ERRNO_P(addr));
        return -ENOMEM;
    }
    ...
    if (ret < 0) {
        log_error("ECREATE failed in enclave creation ioctl (errno = %d)\n", ret);
        return ret;
    }

    if (ret) {
        log_error("ECREATE failed (errno = %d)\n", ret);
        return -EPERM;
    }
    ...
    log_debug("    size:           0x%016lx\n", secs->size);
    log_debug("    misc_select:    0x%08x\n",   secs->misc_select);
    log_debug("    attr.flags:     0x%016lx\n", secs->attributes.flags);
    log_debug("    attr.xfrm:      0x%016lx\n", secs->attributes.xfrm);
    log_debug("    ssa_frame_size: %d\n",       secs->ssa_frame_size);
    log_debug("    isv_prod_id:    0x%08x\n",   secs->isv_prod_id);
    log_debug("    isv_svn:        0x%08x\n",   secs->isv_svn);
...
int add_pages_to_enclave(sgx_arch_secs_t* secs, void* addr, void* user_addr, unsigned long size,
    ...
    if (size == g_page_size)
        log_debug("adding page  to enclave: %p [%s:%s] (%s)%s\n", addr, t, p, comment, m);
    else
        log_debug("adding pages to enclave: %p-%p [%s:%s] (%s)%s\n", addr, addr + size, t, p,
                  comment, m);
    ...
    if (!user_addr && g_zero_pages_size < size) {
        ...
        g_zero_pages = (void*)INLINE_SYSCALL(mmap, 6, NULL, size, PROT_READ,
                                             MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
        if (IS_ERR_P(g_zero_pages)) {
            log_error("Cannot map zero pages %ld\n", ERRNO_P(g_zero_pages));
            return -ENOMEM;
        }
    ...
    while (param.length > 0) {
        ...
        if (ret < 0) {
            if (ret == -EINTR)
                continue;
            log_error("Enclave EADD returned %d\n", ret);
            return ret;
        }
    ...
    if (ret < 0) {
        log_error("Changing protections of EADDed pages returned %d\n", ret);
        return ret;
    }
...
int init_enclave(sgx_arch_secs_t* secs, sgx_arch_enclave_css_t* sigstruct,
    ...
    if (ret) {
        ...
        log_error("enclave EINIT failed - %s\n", error);
        return -EPERM;
    }
