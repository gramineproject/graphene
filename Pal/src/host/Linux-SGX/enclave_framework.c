...
static int file_policy = FILE_POLICY_DEFAULT;
                       ...
                       uint64_t * sizeptr, int create)
{
    ...
    if (!tf || !tf->index) {
        if (!tf) {
            if (get_file_policy() != FILE_POLICY_LOG)
                return -PAL_ERROR_DENIED;

            SGX_DBG(DBG_I, "Allowing access to an unknown file due to file_policy settings: %s\n",
                    uri);
        }
...
int init_file_policy (void)
{
    ...
    if (ret > 0) {
        ...
        else if (strcmp_static(cfgbuf, "log"))
            set_file_policy(FILE_POLICY_LOG);
        else
