 ...
 */
static int sanitize_hw_resource_range(PAL_RES_RANGE_INFO* res_info, uint64_t res_min_limit,
                                      uint64_t res_max_limit, uint64_t range_min_limit,
...
static int sanitize_cache_topology_info(PAL_CORE_CACHE_INFO* cache, uint64_t cache_lvls,
    ...
    for (uint64_t lvl = 0; lvl < cache_lvls; lvl++) {
        ...
        uint64_t level = cache[lvl].level;
        ...
        uint64_t coherency_line_size = cache[lvl].coherency_line_size;
        ...
        uint64_t physical_line_partition = cache[lvl].physical_line_partition;
...
static int sanitize_core_topology_info(PAL_CORE_TOPO_INFO* core_topology, uint64_t num_cores,
                                       uint64_t cache_lvls) {
    int ret;
    if (num_cores == 0 || cache_lvls == 0)
        return -1;

    for (uint64_t idx = 0; idx < num_cores; idx++) {
        if (idx != 0) {     /* core 0 is always online */
            uint64_t is_core_online = core_topology[idx].is_logical_core_online;
            if (is_core_online != 0 && is_core_online != 1)
                return -1;
        }

        uint64_t core_id = core_topology[idx].core_id;
...
static int sanitize_socket_info(PAL_TOPO_INFO* topo_info) {
    int ret = 0;
    uint64_t prev_socket = UINT32_MAX;

    uint64_t num_sockets = topo_info->num_sockets;
    PAL_RES_RANGE_INFO* socket_info =
        (PAL_RES_RANGE_INFO*)calloc(num_sockets, sizeof(PAL_RES_RANGE_INFO));
    if (!socket_info)
        return -1;

    uint64_t num_cores = topo_info->online_logical_cores.resource_count;
    for (uint64_t idx = 0; idx < num_cores; idx++) {
        uint64_t socket = (int64_t)topo_info->core_topology[idx].cpu_socket;
        if (socket > num_sockets - 1) {
            ret = -1;
            goto out_socket;
        }
        ...
        if (socket != prev_socket) {
            ...
            if (socket_info[socket].ranges) {
                memcpy(tmp, socket_info[socket].ranges, old_sz);
                free(socket_info[socket].ranges);
            }
            socket_info[socket].ranges = tmp;

            uint64_t range_idx = socket_info[socket].range_count - 1;
    ...
    for (uint64_t idx = 0; idx < num_sockets; idx++) {
        ...
        uint64_t core_sibling_cnt =
                                topo_info->core_topology[core_in_socket].core_siblings.range_count;
...
static int sanitize_numa_topology_info(PAL_NUMA_TOPO_INFO* numa_topology, uint64_t num_nodes,
                                       uint64_t num_cores, uint64_t possible_cores) {
    int ret = 0;
    ...
    uint64_t total_cores_in_numa = 0;
    for (uint64_t idx = 0; idx < num_nodes; idx++) {
        ...
        for (uint64_t i = 0; i < numa_topology[idx].cpumap.range_count; i++) {
...
static int sgx_copy_core_topo_to_enclave(PAL_CORE_TOPO_INFO* src, uint64_t online_logical_cores,
                                         uint64_t num_cache_index) {
    ...
    for (uint64_t idx = 0; idx < online_logical_cores; idx++) {
        ...
        for (uint64_t lvl = 0; lvl < num_cache_index; lvl++) {
...
static int sgx_copy_numa_topo_to_enclave(PAL_NUMA_TOPO_INFO* src, uint64_t num_online_nodes) {
    ...
    for (uint64_t idx = 0; idx < num_online_nodes; idx++) {
...
static int parse_host_topo_info(PAL_TOPO_INFO* topo_info) {
    ...
    uint64_t online_logical_cores = g_pal_sec.topo_info.online_logical_cores.resource_count;
    ...
    uint64_t possible_cores = g_pal_sec.topo_info.possible_logical_cores.resource_count;
    uint64_t num_online_nodes = g_pal_sec.topo_info.nodes.resource_count;
