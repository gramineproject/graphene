...
static int collect_sub_regions(toml_array_t* root_toml_array, void* root_encl_addr,
    ...
    struct mem_region* mem_regions = (struct mem_region*)get_tcb_trts()->ioctl_scratch_space;
 ...
 *   3. Ioctl-struct cache of size IOCTL_STRUCT_CACHE_SIZE +
 *   4. Subregions cache of size SUB_REGION_INFO_CACHE_SIZE
 *
...
static int init_ioctl_scratch_space(void) {
    if (!get_tcb_trts()->ioctl_scratch_space) {
        size_t total_size = 0;
        total_size += MAX_MEM_REGIONS * sizeof(struct mem_region);
