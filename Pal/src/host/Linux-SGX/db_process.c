 ...
 *       collaterals are used to identify the connection (via SHA256(g_x || g_y) for parent enclave
 *       A and via SHA256(g_y || g_x) for child enclave B to prevent man-in-the-middle/interleaving
 *       attacks) as well as to derive the session key for secure IPC.
 *
 ...
 *       contain a secure hash over DH collaterals: SHA256(g_x || g_y) for parent enclave A and
 *       SHA256(g_y || g_x) for child enclave B (the reverse orders are required to prevent
 *       interleaving attacks). Because a new DH key exchange protocol is run for each enclave pair,
 *       no report can be reused even from an enclave with the same MR_ENCLAVE.
 */
...
int _DkProcessCreate(PAL_HANDLE* handle, const char** args) {
    ...
    ret = _DkStreamKeyExchange(child, &child->process.session_key, (uint8_t*)&parent_report_data,
                               (uint8_t*)&child_report_data);
    if (ret < 0)
        goto failed;
...
int init_child_process(PAL_HANDLE* parent_handle) {
    ...
    int ret = _DkStreamKeyExchange(parent, &parent->process.session_key,
                                   (uint8_t*)&child_report_data, (uint8_t*)&parent_report_data);
    if (ret < 0)
        return ret;
