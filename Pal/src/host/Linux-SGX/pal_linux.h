...
/* perform Diffie-Hellman to establish a session key and also produce a hash over (g_x || g_y) for
 * parent enclave A and over (g_y || g_x) for child enclave B */
 ...
 * \param  expected_data  Expected SGX report data, contains SHA256(g_x || g_y) (or reverse).
 * \param  peer_data      Peer enclave's SGX report data, contains SHA256(g_x || g_y) (or reverse).
 * \return 0 on success, negative error code otherwise.
 */
