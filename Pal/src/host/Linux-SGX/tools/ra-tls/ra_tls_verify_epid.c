...
int ra_tls_verify_callback(void* data, mbedtls_x509_crt* crt, int depth, uint32_t* flags) {
    ...
    /* note that the below function verifies `isvEnclaveQuoteStatus` returned in the IAS attestation
     * report; it fails if the SGX quote is invalid or if the EPID group/private key/signature is
     * revoked (see also ttps://api.trustedservices.intel.com/documents/IAS-API-Spec-rev-5.0.pdf) */
