...
#define RA_TLS_REPORT_URL   "RA_TLS_REPORT_URL"
#define RA_TLS_SIGRL_URL    "RA_TLS_SIGRL_URL"
...
/*!
 * \brief mbedTLS-suitable verification callback for EPID-based (IAS) quote verification.
 *
 * This callback must be registered via mbedtls_ssl_conf_verify(). All parameters required for
 * the SGX quote and IAS attestation report verification must be passed in the corresponding
...
/*!
 * \brief Generic verification callback for EPID-based (IAS) quote verification.
 *
 ...
 * quote and IAS attestation report verification must be passed in the corresponding RA-TLS
