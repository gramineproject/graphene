...
int ra_tls_verify_callback(void* data, mbedtls_x509_crt* crt, int depth, uint32_t* flags) {
    ...
    if (!report_data || !report_data_size || !sig_data || !sig_data_size) {
        ERROR("Received IAS attestation report doesn't contain report and/or signature\n");
        ret = -1;
        goto out;
    }
    ...
    if (mrsigner_hex) {
        if (parse_hex(mrsigner_hex, &expected_mrsigner, sizeof(expected_mrsigner)) != 0) {
            ERROR("Failed to parse mrsigner environment variable (must be a hex string)\n");
            ret = -1;
            goto out;
        }
    }
    ...
    if (mrenclave_hex) {
        if (parse_hex(mrenclave_hex, &expected_mrenclave, sizeof(expected_mrenclave)) != 0) {
            ERROR("Failed to parse mrenclave environment variable (must be a hex string)\n");
            ret = -1;
            goto out;
        }
    }
