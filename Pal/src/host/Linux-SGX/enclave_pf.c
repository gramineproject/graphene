...
static int register_protected_files(void) {
    ...
    /* read sgx.protected_files table from manifest */
...
int init_protected_files(void) {
    ...
    toml_raw_t protected_files_key_raw = manifest_sgx
                                         ? toml_raw_in(manifest_sgx, "protected_files_key")
                                         : NULL;

    if (protected_files_key_raw) {
        char* protected_files_key_str = NULL;
        ret = toml_rtos(protected_files_key_raw, &protected_files_key_str);
        if (ret < 0) {
            SGX_DBG(DBG_E, "Cannot read \'sgx.protected_files_key\' (it must be put in quotes!)\n");
