...
int init_protected_files() {
    // initialize protected files library
...
struct protected_file* load_protected_file(const char* path, int* fd, size_t size,
    ...
    SGX_DBG(DBG_D, "load_protected_file: %s, fd %d, size %lu, mode %d, create %d\n",
            path, *fd, size, mode, create);
    ...
    if (!pf) {
        ...
        if (pf) {
            SGX_DBG(DBG_D, "registering new pf file %s in dir %s\n", path, pf->path);
            if (register_protected_path(path, &pf) < 0)
                return NULL;
        }
    }
...
int unload_protected_file(struct protected_file* pf) {
    ...
    // Flush/free any remaining buffers
