...
int main (int argc, char ** argv, char ** envp)
{
    ...
    fd = INLINE_SYSCALL(open, 3, exec_uri + static_strlen("file:"),
                        O_RDONLY|O_CLOEXEC, 0);
    if (IS_ERR(fd)) {
    ...
    char filebuf[4];
    /* Check if the first argument is a executable */
    ...
    if (memcmp(filebuf, "\177ELF", 4)) {
        // In this case the manifest is given as the executable.
        // Drop the .manifest* from exec_uri, so that the program
        // loads properly.
        ...
        /* Try finding the possible manifest file */
        strcpy_static(sgx_manifest + len, ".manifest.sgx", sizeof(sgx_manifest) - len);

        fd = manifest_fd = INLINE_SYSCALL(open, 3, sgx_manifest, O_RDONLY|O_CLOEXEC, 0);
    ...
    manifest_uri = alloc_concat("file:", static_strlen("file:"), sgx_manifest, -1);
    if (!manifest_uri) {
        ret = -ENOMEM;
        goto out;
    }
    ...
    ret = load_enclave(enclave, manifest_fd, manifest_uri, exec_uri, args, args_size,
