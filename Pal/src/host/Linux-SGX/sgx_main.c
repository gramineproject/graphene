...
int main (int argc, char ** argv, char ** envp)
{
    ...
    INLINE_SYSCALL(read, 3, fd, exec_first_four_bytes,
                   sizeof(exec_first_four_bytes));

    int manifest_fd;
    char exec_uri_base_name[URI_MAX];
    size_t len = sizeof(exec_uri_base_name);
    ret = get_base_name(exec_uri + static_strlen("file:"),
                        exec_uri_base_name, &len);
    if (ret < 0) {
        goto out;
    }

    if (memcmp(exec_first_four_bytes, "\177ELF",
               sizeof(exec_first_four_bytes))) {
        ...
        fd = manifest_fd = INLINE_SYSCALL(open, 3, exec_uri_base_name,
                                          O_RDONLY|O_CLOEXEC, 0);
