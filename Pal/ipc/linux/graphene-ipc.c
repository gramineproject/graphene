...
static struct miscdevice gipc_dev = {
    .minor = GIPC_MINOR, .name = "gipc", .fops = &gipc_fops, .mode = 0666,
};
...
static int __init gipc_init(void) {
    ...
    gipc_queue_cachep =
        kmem_cache_create("gipc_queue", sizeof(struct gipc_queue), 0, SLAB_HWCACHE_ALIGN |
    ...
    gipc_send_buffer_cachep = kmem_cache_create("gipc_send_buffer", sizeof(struct gipc_send_buffer),
