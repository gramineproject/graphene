...
int main(int argc, char **argv) 
{
	...
	if (argc == 1) {
		...
		for (i = 0; i < TEST_PAGES; i++) {
			...
			memset(pages[i] + (TEST_PAGES-7) * PAGE_SIZE, 'b' + i, 256);
			pages[i][256] = '\0';

			memset(pages[i] + (TEST_PAGES-1) * PAGE_SIZE, 'c' + i, 256);
			pages[i][256] = '\0';
		...
		for (i = 0; i < max; i++) {
			...
			printf("%s\n", (char *) map.addr);
			printf("%s\n", (char *) map.addr + (TEST_PAGES -7)*PAGE_SIZE);
			printf("%s\n", (char *) map.addr + (TEST_PAGES -1)*PAGE_SIZE);

			/* Now touch some of the pages, which was not sent by parent */
			empty_zone = (char *) (map.addr + 2*PAGE_SIZE);
			memset(empty_zone, 'd' + i, 256);
			empty_zone[256]=0;

			printf("content at empty zone %s\n", (char *) empty_zone);
