...
int main(int argc, const char** argv, const char** envp) {
    ...
    if (r == -1 && errno == EFAULT)
        printf("execv(invalid-path) correctly returned error\n");

    r = execve(argv[0], bad_argv, good_envp);
    if (r == -1 && errno == EFAULT)
        printf("execv(invalid-argv) correctly returned error\n");

    r = execve(argv[0], good_argv, bad_envp);
    if (r == -1 && errno == EFAULT)
        printf("execv(invalid-envp) correctly returned error\n");
