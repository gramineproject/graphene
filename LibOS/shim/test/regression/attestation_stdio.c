/* Attestation API test (using stdio's fopen/fread/...). Only works for SGX PAL. */
...
static int test_resource_leak(void) {
    /* repeatedly fopen()/fclose() pseudo-files to hopefully uncover resource leaks */
    for (int j = 0; j < sizeof(paths) / sizeof(&paths[0]); j++) {
        for (int i = 0; i < 100000; i++) {
            FILE* f = fopen(paths[j], "r");
            if (!f) {
