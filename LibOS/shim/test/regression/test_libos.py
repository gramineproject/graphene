...
class TC_00_Unittests(RegressionTestCase):
    def test_000_spinlock(self):
        stdout, stderr = self.run_binary(['spinlock']) # pylint: disable=unused-variable
...
class TC_01_Bootstrap(RegressionTestCase):
    ...
    def test_102_basic_bootstrapping_static(self):
        # bootstrap_static
        stdout, stderr = self.run_binary(['bootstrap_static']) # pylint: disable=unused-variable
        self.assertIn('Hello world (bootstrap_static)!', stdout)

    def test_103_basic_bootstrapping_pie(self):
        # bootstrap_pie
        stdout, stderr = self.run_binary(['bootstrap_pie']) # pylint: disable=unused-variable
        self.assertIn('User program started', stdout)
    ...
    def test_110_basic_bootstrapping_cxx(self):
        stdout, stderr = self.run_binary(['bootstrap-c++']) # pylint: disable=unused-variable

        # Basic Bootstrapping (C++)
        self.assertIn('User Program Started', stdout)

    def test_200_exec(self):
        stdout, stderr = self.run_binary(['exec']) # pylint: disable=unused-variable
    ...
    def test_201_exec_same(self):
        stdout, stderr = self.run_binary(['exec_same']) # pylint: disable=unused-variable
        self.assertIn('hello from execv process', stdout)

    def test_202_fork_and_exec(self):
        stdout, stderr = self.run_binary(['fork_and_exec']) # pylint: disable=unused-variable
    ...
    def test_203_vfork_and_exec(self):
        stdout, stderr = self.run_binary(['vfork_and_exec']) # pylint: disable=unused-variable
    ...
    def test_210_exec_invalid_args(self):
        stdout, stderr = self.run_binary(['exec_invalid_args']) # pylint: disable=unused-variable
    ...
    def test_300_shared_object(self):
        stdout, stderr = self.run_binary(['shared_object']) # pylint: disable=unused-variable
    ...
    def test_600_multi_pthread(self):
        stdout, stderr = self.run_binary(['multi_pthread']) # pylint: disable=unused-variable
...
class TC_02_OpenMP(RegressionTestCase):
    def test_000_simple_for_loop(self):
        stdout, stderr = self.run_binary(['openmp']) # pylint: disable=unused-variable
...
class TC_03_FileCheckPolicy(RegressionTestCase):
    def test_000_strict_success(self):
        ...
        stdout, stderr = self.run_binary([manifest, 'trusted_testfile']) # pylint: disable=unused-variable
    ...
    def test_001_strict_fail(self):
        manifest = self.get_manifest('file_check_policy_strict')
        try:
            stdout, stderr = self.run_binary([manifest, 'unknown_testfile']) # pylint: disable=unused-variable
...
class TC_30_Syscall(RegressionTestCase):
    def test_000_getcwd(self):
        stdout, stderr = self.run_binary(['getcwd']) # pylint: disable=unused-variable
    ...
    def test_010_stat_invalid_args(self):
        stdout, stderr = self.run_binary(['stat_invalid_args']) # pylint: disable=unused-variable
    ...
    def test_011_fstat_cwd(self):
        stdout, stderr = self.run_binary(['fstat_cwd']) # pylint: disable=unused-variable
    ...
    def test_020_getdents(self):
        ...
        stdout, stderr = self.run_binary(['getdents']) # pylint: disable=unused-variable
    ...
    def test_021_getdents_large_dir(self):
        stdout, stderr = self.run_binary(['large_dir_read', 'tmp/large_dir', '3000']) # pylint: disable=unused-variable
    ...
    def test_030_fopen(self):
        ...
        stdout, stderr = self.run_binary(['fopen_cornercases']) # pylint: disable=unused-variable
    ...
    def test_050_mmap(self):
        stdout, stderr = self.run_binary(['mmap-file'], timeout=60) # pylint: disable=unused-variable
    ...
    def test_052_large_mmap(self):
        stdout, stderr = self.run_binary(['large-mmap'], timeout=480) # pylint: disable=unused-variable
    ...
    def test_060_sigaltstack(self):
        stdout, stderr = self.run_binary(['sigaltstack']) # pylint: disable=unused-variable
    ...
    def test_070_eventfd(self):
        stdout, stderr = self.run_binary(['eventfd']) # pylint: disable=unused-variable
    ...
    def test_080_sched(self):
        stdout, stderr = self.run_binary(['sched']) # pylint: disable=unused-variable
...
class TC_31_SyscallSGX(RegressionTestCase):
    def test_000_syscall_redirect(self):
        stdout, stderr = self.run_binary(['syscall']) # pylint: disable=unused-variable
...
class TC_40_FileSystem(RegressionTestCase):
    def test_000_base(self):
        stdout, stderr = self.run_binary(['proc']) # pylint: disable=unused-variable
    ...
    def test_010_path(self):
        stdout, stderr = self.run_binary(['proc-path']) # pylint: disable=unused-variable

        # Base /proc path present
        self.assertIn('proc path test success', stdout)

    def test_020_cpuinfo(self):
        stdout, stderr = self.run_binary(['proc_cpuinfo'], timeout=50) # pylint: disable=unused-variable

        # proc/cpuinfo Linux-based formatting
        self.assertIn('cpuinfo test passed', stdout)

    def test_030_fdleak(self):
        stdout, stderr = self.run_binary(['fdleak'], timeout=10) # pylint: disable=unused-variable
...
class TC_80_Socket(RegressionTestCase):
    def test_000_getsockopt(self):
        stdout, stderr = self.run_binary(['getsockopt']) # pylint: disable=unused-variable
        self.assertIn('getsockopt: Got socket type OK', stdout)

    def test_010_epoll_wait_timeout(self):
        stdout, stderr = self.run_binary(['epoll_wait_timeout', '8000'], # pylint: disable=unused-variable
            timeout=50)
    ...
    def test_100_socket_unix(self):
        stdout, stderr = self.run_binary(['unix']) # pylint: disable=unused-variable
    ...
    def test_200_socket_udp(self):
        stdout, stderr = self.run_binary(['udp'], timeout=50) # pylint: disable=unused-variable
        self.assertIn('Data: This is packet 0', stdout)
        self.assertIn('Data: This is packet 1', stdout)
        self.assertIn('Data: This is packet 2', stdout)
        self.assertIn('Data: This is packet 3', stdout)
