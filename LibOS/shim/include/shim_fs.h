...
#define DENTRY_ANCESTOR                                \
    0x4000 /* Auto-generated dentry to connect a mount \
...
struct shim_dentry {
     ...
     */
    LIST_TYPE(shim_dentry)
    hlist; /* to resolve collisions in
             the hash table */
    LIST_TYPE(shim_dentry)
    list; /* put dentry to different list
...
enum lookup_type { LAST_NORM, LAST_ROOT, LAST_DOT, LAST_DOTDOT, LAST_BIND };
...
static_always_inline void fast_pathcpy(char* dst, const char* src, size_t size, char** ptr) {
    char* d       = dst;
    const char* s = src;
    for (size_t i = 0; i < size; i++, s++, d++) *d = *s;
    *ptr = d;
}
