...
/* This function extracts first number from the file path. For example, "3" will be extracted
 * from "cpu/cpu3/topology/core_siblings" path */
...
/* This function checks if directory present in the pathname is valid. Returns 1 on success and 0
 * on failure. */
int sys_match_resource_num(const char* pathname);
/* This function lists directory entries present in the pathname. Returns 0 on success and negative
 * UNIX error on failure. */
