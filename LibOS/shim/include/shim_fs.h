...
/*!
 * \brief Allocate and initialize a new dentry for path name, under parent. The caller should hold
 * `g_dcache_lock`.
 *
 ...
 *
 * The function will initialize `fs` (if provided), `name`, `rel_path`, and parent/children links.
 *
 * The reference count of the returned dentry will be 2 if `parent` was provided, 1 otherwise.
 *
 * `fs` should typically be `parent->fs`, but is passed explicitly to support initializing the root
 * dentry of a newly mounted filesystem. If `fs` is NULL, the resulting dentry's filesystem will be
 * left as NULL.
 *
...
/*!
 * \brief Search for a child of a dentry with a given name. The caller should hold `g_dcache_lock`.
 *
