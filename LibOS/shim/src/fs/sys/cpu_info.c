...
int sys_cpu_load(struct shim_dentry* dent, char** out_data, size_t* out_size) {
    ...
    if (strcmp(name, "online") == 0) {
        ...
        if (cpu_num == 0)
            return -ENOENT;
        ret = sys_convert_int_to_str(core_topology->is_logical_core_online, MULTIPLIER_NONE, str,
                                     sizeof(str));
    } else if (strcmp(name, "core_id") == 0) {
        ret = sys_convert_int_to_str(core_topology->core_id, MULTIPLIER_NONE, str, sizeof(str));
