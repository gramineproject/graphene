...
int sys_cache_load(struct shim_dentry* dent, char** out_data, size_t* out_size) {
    ...
    if (strcmp(name, "shared_cpu_map") == 0) {
        ...
        ret = sys_convert_int_to_str(cache->size, cache->size_multiplier, str, sizeof(str));
    } else if (strcmp(name, "coherency_line_size") == 0) {
        ret = sys_convert_int_to_str(cache->coherency_line_size, MULTIPLIER_NONE, str, sizeof(str));
    } else if (strcmp(name, "number_of_sets") == 0) {
        ret = sys_convert_int_to_str(cache->number_of_sets, MULTIPLIER_NONE, str, sizeof(str));
