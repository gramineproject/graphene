 ...
 * fs.c
 *
 * This file contains the implementation of the 'sys' filesystem.
 */
...
const struct proc_dir sys_root = {
    .size = 1,
    .ent =
        {
            {
...
#define SYS_INO_BASE 1

static int sys_root_mode(const char* name, mode_t* mode) {
    __UNUSED(name);  // We know this is /proc
    *mode = 0555;
    return 0;
}

static int sys_root_stat(const char* name, struct stat* buf) {
    __UNUSED(name);  // We know this is /proc
...
static int sys_root_open(struct shim_handle* hdl, const char* name, int flags) {
...
struct proc_fs_ops fs_sys_root = {
    .open = &sys_root_open,
    .mode = &sys_root_mode,
    .stat = &sys_root_stat,
};

const struct proc_ent sys_root_ent = {
    .name   = "",
    .fs_ops = &fs_sys_root,
    .dir    = &sys_root,
};
...
static int sys_match_name(const char* trim_name, const struct proc_ent** found_ent) {
    if (!trim_name || !trim_name[0]) {
        *found_ent = &sys_root_ent;
        return 0;
    }

    const char* token          = trim_name;
    const char* next_token     = NULL;
    const struct proc_dir* dir = &sys_root;
...
static int sys_mode(struct shim_dentry* dent, mode_t* mode) {
    ...
    int ret = sys_match_name(rel_path, &ent);
...
static int sys_lookup(struct shim_dentry* dent) {
    ...
    /* don't care about forced or not */
    const struct proc_ent* ent = NULL;
    int ret                    = sys_match_name(qstrgetstr(&dent->rel_path), &ent);
...
static int sys_mount(const char* uri, void** mount_data) {
...
static int sys_unmount(void* mount_data) {
    // Arguments for compatibility with other FSes
    __UNUSED(mount_data);
    /* do nothing */
    return 0;
...
static int sys_open(struct shim_handle* hdl, struct shim_dentry* dent, int flags) {
    ...
    if ((ret = sys_match_name(rel_path, &ent)) < 0)
        return ret;
...
static int sys_follow_link(struct shim_dentry* dent, struct shim_qstr* link) {
    const char* rel_path = qstrgetstr(&dent->rel_path);
    const struct proc_ent* ent;
    int ret;
