...
int proc_self_follow_link(struct shim_dentry* dent, struct shim_qstr* link) {
    ...
    if (!qstrsetstr(link, name, strlen(name)))
        return -ENOMEM;
    return 0;
}
...
static void init_ipc_thread_dir(struct pseudo2_ent* ent) {
...
int init_procfs(void) {
    ...
    pseudo_add_str(root, "meminfo", &proc_meminfo_get_content);
    pseudo_add_str(root, "cpuinfo", &proc_cpuinfo_get_content);
    ...
    struct pseudo2_ent* ipc_thread_pid = pseudo_add_dir(root, NULL);
