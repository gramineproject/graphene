...
static int mount_fs_at_dentry(const char* type, const char* uri, const char* mount_path,
    ...
    /* Initialize root of the new filesystem */
    ...
    /* Trigger filesystem lookup for the root dentry, so that we fail the mount operation now if the
...
int mount_fs(const char* type, const char* uri, const char* mount_path, bool make_synthetic) {
    ...
    if (make_synthetic)
        lookup_flags |= LOOKUP_CREATE | LOOKUP_MAKE_SYNTHETIC;
    if ((ret = _path_lookupat(g_dentry_root, mount_path, lookup_flags, &mount_point)) < 0) {
        log_warning("error looking up mountpoint %s: %d\n", mount_path, ret);
        goto out;
    }

    if (mount_point->state & DENTRY_NEGATIVE) {
        assert(make_synthetic);

        mount_point->state &= ~DENTRY_NEGATIVE;
        mount_point->state |= DENTRY_SYNTHETIC | DENTRY_VALID;
    }
