...
static int chroot_open(struct shim_handle* hdl, struct shim_dentry* dent, int flags) {
    ...
    loff_t size = __atomic_load_n(&data->size.counter, __ATOMIC_SEQ_CST);
...
static loff_t chroot_seek(struct shim_handle* hdl, loff_t offset, int whence) {
    loff_t ret = -EINVAL;
...
static int chroot_truncate(struct shim_handle* hdl, loff_t len) {
    int ret = 0;
