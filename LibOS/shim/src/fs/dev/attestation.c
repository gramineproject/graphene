...
static void update_buffer(struct shim_handle* hdl, char* buffer, size_t max_size) {
   assert(hdl->type == TYPE_STR);
   assert(hdl->info.str.data->len >= 0);
   assert(max_size > 0);

   size_t size = hdl->info.str.data->len;
   if (size < max_size) {
       memcpy(buffer, hdl->info.str.data->str, size);
       memset(buffer + size, 0, max_size - size);
   } else {
       memcpy(buffer, hdl->info.str.data->str, max_size);
   }
}
...
static int user_report_data_modify(struct shim_handle* hdl) {
    update_buffer(hdl, g_user_report_data, g_user_report_data_size);
    return 0;
}
...
static int target_info_modify(struct shim_handle* hdl) {
    update_buffer(hdl, g_target_info, g_target_info_size);
    return 0;
}
 ...
 * In case of SGX, target info is an opaque blob of size 512B.
 */
static int my_target_info_get_content(struct shim_dentry* dent, char** content, size_t* size) {
    __UNUSED(dent);
 ...
 */
static int report_get_content(struct shim_dentry* dent, char** content, size_t* size) {
    __UNUSED(dent);
 ...
 */
static int quote_get_content(struct shim_dentry* dent, char** content, size_t* size) {
    __UNUSED(dent);
...
static int pfkey_get_content(struct shim_dentry* dent, char** content, size_t* size) {
    __UNUSED(dent);
...
static int pfkey_modify(struct shim_handle* hdl) {
    update_buffer(hdl, g_pf_key_hex, sizeof(g_pf_key_hex) - 1);
...
int init_attestation(struct pseudo2_ent* dev) {
    ...
    struct pseudo2_ent* attestation = pseudo_add_dir(dev, "attestation");

    g_user_report_data_size = USER_REPORT_DATA_MAX_SIZE;
    g_target_info_size = TARGET_INFO_MAX_SIZE;

    struct pseudo2_ent* user_report_data = pseudo_add_str(attestation, "user_report_data", NULL);
    user_report_data->perm = PSEUDO_MODE_FILE_RW;
    user_report_data->str.modify = &user_report_data_modify;

    struct pseudo2_ent* target_info = pseudo_add_str(attestation, "target_info", NULL);
    target_info->perm = PSEUDO_MODE_FILE_RW;
    target_info->str.modify = &target_info_modify;

    pseudo_add_str(attestation, "my_target_info", &my_target_info_get_content);
    pseudo_add_str(attestation, "report", &report_get_content);
    pseudo_add_str(attestation, "quote", &quote_get_content);

    struct pseudo2_ent* pfkey = pseudo_add_str(attestation, "protected_files_key",
                                               &pfkey_get_content);
    pfkey->perm = PSEUDO_MODE_FILE_RW;
    pfkey->str.modify = &pfkey_modify;
    return 0;
}
