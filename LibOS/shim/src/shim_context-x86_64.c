...
void shim_xstate_save(struct shim_xstate* xstate) {
    ...
    /* Emulate format for FP registers that Linux uses for x86-64:
     ...
     * similar to Linux's check_for_xstate() */
    ...
    if (g_shim_xsave_enabled) {
        /* for XSAVE, the last 32-bit word of the total xstate area (at the fpstate + extended_size
         * - FP_XSTATE_MAGIC2_SIZE address) is set to FP_XSTATE_MAGIC2 so that app/Graphene can
         * sanity check XSAVE size calculations */
        *((__typeof__(SHIM_FP_XSTATE_MAGIC2)*)((char*)xstate + fpx_sw->xstate_size)) =
...
void shim_xstate_reset(void) {
    shim_xstate_restore((struct shim_xstate*)g_shim_xstate_reset_state);
}
