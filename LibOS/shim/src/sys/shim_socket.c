...
int shim_do_bind(int sockfd, struct sockaddr* addr, int _addrlen) {
    ...
    if (sock->domain == AF_UNIX) {
        ...
        if ((ret = path_lookupat(NULL, spath, LOOKUP_CREATE, &dent, NULL)) < 0) {
            /* DEP 7/3/17: We want either 0 or -ENOENT (dent is a valid object in both cases), as
             * the expected case is that the name is free (and we use dent with the name already
             * populated). FIXME: This is terrible semantics; path_lookupat() must be re-worked. */
            if (ret != -ENOENT || !dent)
                goto out;
        }
