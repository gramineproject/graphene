...
int init_ipc_ids(void) {
...
#ifdef DO_TESTS
    struct id_range* range1 = malloc(sizeof(*range1));
    if (!range1) {
        return -ENOMEM;
    }
    struct id_range* range2 = malloc(sizeof(*range2));
    if (!range2) {
        return -ENOMEM;
    }
    range1->start = 34;
    range1->end = 0x1334;
    range1->owner = 0;
    avl_tree_insert(&g_id_owners_tree, &range1->node);
    range2->start = 0x1337;
    range2->end = UINT32_MAX - 7;
    range2->owner = 0;
    avl_tree_insert(&g_id_owners_tree, &range2->node);
    //g_last_id = range2->end;
#endif // DO_TESTS
    return 0;
}
...
int ipc_alloc_id_range(IDTYPE* start, IDTYPE* end) {
    if (!g_process_ipc_ids.leader_vmid) {
        return alloc_id_range(g_self_vmid, start, end);
    }
    ...
    init_ipc_msg(msg, IPC_MSG_NEW_ID_RANGE, msg_size);
