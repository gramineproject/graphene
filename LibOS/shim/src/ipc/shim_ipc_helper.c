 ...
 * consults before each new DkStreamsWaitEvents().
 *
 * Note that ports are copied from global port_list to local ports. This
 * is because ports may be removed from port_list by other threads while IPC
 * helper thread is waiting on DkStreamsWaitEvents(). For this reason IPC thread
