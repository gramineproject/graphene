...
static int receive_ipc_messages(struct shim_ipc_connection* conn) {
     ...
     * `0x40` as a random estimation of "couple of ints" + message header size to get it if
     * possible. */
...
#undef READAHEAD_SIZE
    ...
    do {
        ...
        unsigned long seq = buf.msg_header.seq;
        ...
        log_debug(LOG_PREFIX "received IPC message from %u: code=%d size=%lu seq=%lu\n", conn->vmid,
                  msg_code, msg_size, seq);

        if (msg_code < ARRAY_SIZE(ipc_callbacks) && ipc_callbacks[msg_code]) {
            int ret = ipc_callbacks[msg_code](conn->vmid, msg_data, seq);
            if ((ret < 0 || ret == RESPONSE_CALLBACK) && seq) {
                ret = send_ipc_response(conn->vmid, ret, seq);
                if (ret < 0) {
