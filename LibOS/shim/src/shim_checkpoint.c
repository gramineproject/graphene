...
static int receive_handles_on_stream(struct checkpoint_hdr* hdr, void* base, ssize_t rebase) {
    ...
    for (size_t i = 0; i < entries_cnt; i++) {
        ...
        PAL_HANDLE hdl = NULL;
        ret = DkReceiveHandle(DkGetPalControl()->parent_process, &hdl);
...
int create_process_and_send_checkpoint(migrate_func_t migrate_func,
    ...
    const char* exec_uri = DkGetPalControl()->executable;
...
int receive_checkpoint_and_restore(struct checkpoint_hdr* hdr) {
    ...
    if (pal_control->user_address.start <= mapaddr &&
            mapaddr + mapsize <= pal_control->user_address.end) {
    ...
    ret = read_exact(pal_control->parent_process, base, hdr->size);
    if (ret < 0) {
        goto out_fail;
    }
    log_debug("read checkpoint of %lu bytes from parent\n", hdr->size);

    ret = receive_memory_on_stream(pal_control->parent_process, hdr, (uintptr_t)base);
    if (ret < 0) {
        goto out_fail;
    }
