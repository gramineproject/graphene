...
static int read_file_fragment(struct shim_handle* file, void* buf, size_t size, loff_t offset);
...
static int read_file_fragment(struct shim_handle* file, void* buf, size_t size, loff_t offset) {
    ...
    loff_t (*seek)(struct shim_handle*, loff_t, int)    = file->fs->fs_ops->seek;

    if (!read || !seek)
        return -EACCES;

    loff_t seek_ret = seek(file, offset, SEEK_SET);
    if (seek_ret < 0)
        return seek_ret;
