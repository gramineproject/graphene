...
static int read_loadcmd(const ElfW(Phdr*) ph, struct loadcmd *c) {
    ...
    if (!IS_ALLOC_ALIGNED(ph->p_align)) {
        log_debug("read_loadcmd: ELF load command alignment not page-aligned\n");
        return -EINVAL;
    }

    if (!IS_ALIGNED_POW2(ph->p_vaddr - ph->p_offset, ph->p_align)) {
        log_debug("read_loadcmd: ELF load command address/offset not properly aligned\n");
        return -EINVAL;
    }

    if (ph->p_filesz > ph->p_memsz) {
        log_debug("read_loadcmd: file size larger than memory size\n");
        return -EINVAL;
    }
...
/*
 * Execute a single load command: bookeep the memory, map the file content, and make sure the area
 * not mapped to a file (ph_filesz .. ph_memsz) is zero-filled.
 */
static int execute_loadcmd(const struct loadcmd* c, ElfW(Addr) load_addr, struct shim_handle* file) {
    int ret;
    ...
    if (c->start < c->map_end) {
        ...
        if ((ret = bkeep_mmap_fixed(map_start, map_size, c->prot, MAP_FILE | MAP_FIXED | MAP_PRIVATE,
                                    file, c->map_off, /*comment=*/NULL)) < 0) {
            log_debug("execute_loadcmds: failed to bookkeep address of segment\n");
            return ret;
        }

        if ((ret = file->fs->fs_ops->mmap(file, &map_start, map_size, c->prot,
                                          MAP_FILE | MAP_FIXED | MAP_PRIVATE, c->map_off) < 0)) {
...
static struct link_map* __map_elf_object(struct shim_handle* file, ElfW(Ehdr)* ehdr) {
    ...
    for (ph = phdr; ph < &phdr[ehdr->e_phnum]; ++ph) {
        switch (ph->p_type) {
            ...
            case PT_LOAD:
                if (l->l_nloadcmds >= MAX_LOADCMDS) {
                    errstring = "too many load commands";
                    goto call_lose;
                }

                if ((ret = read_loadcmd(ph, &l->l_loadcmds[l->l_nloadcmds])) < 0) {
                    errstring = "failed to read load command";
                    goto call_lose;
                }
