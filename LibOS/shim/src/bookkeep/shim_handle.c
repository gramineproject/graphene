...
static inline int init_exec_handle(void) {
    const char* executable = DkGetPalControl()->executable;
    if (!executable)
        return 0;
    ...
    if (g_process.exec) {
        ...
        // handling from PAL. `executable` is valid only until first execve - in such case it's
        // stale, and g_process.exec is already initialized by shim_do_execve_rtld.
    ...
    qstrsetstr(&exec->uri, executable, strlen(executable));
    exec->type     = TYPE_FILE;
    exec->flags    = O_RDONLY;
    exec->acc_mode = MAY_READ;

    struct shim_mount* fs = find_mount_from_uri(executable);
    if (fs) {
        const char* p = executable + fs->uri.len;
        /* Lookup for `executable` needs to be done under a given mount point. which requires a
         * relative path name.  On the other hand, the one in manifest file can be absolute path. */
        while (*p == '/') {
            p++;
        }
