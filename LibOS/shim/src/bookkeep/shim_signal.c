...
void pop_allowed_signal(__sigset_t* mask, struct shim_signal* signal) {
    ...
    if (have_forced_signal()) {
        /* forced signals are not artifically generated by kill()/raise()/etc. (i.e., they are
         * generated due to hardware exception) and thus cannot be blocked, so no mask check */
        ...
        unlock(&g_process_signal_queue_lock);
    } else if (!__sigismember(mask ? : &current->signal_mask, SIGTERM) &&
