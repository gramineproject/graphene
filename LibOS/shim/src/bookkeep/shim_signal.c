...
static __rt_sighandler_t __get_sighandler(struct shim_thread* thread, int sig, bool allow_reset) {
    lock(&thread->signal_handles_TODO->lock);
    struct __kernel_sigaction* sig_action = &thread->signal_handles_TODO->actions[sig - 1];
...
# error "__get_sighandler: see the commen above"
    ...
    unlock(&thread->signal_handles_TODO->lock);
    return handler;
}
