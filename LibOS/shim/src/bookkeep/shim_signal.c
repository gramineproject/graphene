...
__handle_one_signal(shim_tcb_t* tcb, int sig, struct shim_signal* signal) {
    ...
    lock(&thread->lock);
    handler = __get_sighandler(thread, sig, /*allowreset=*/true);
    unlock(&thread->lock);
...
void append_signal(struct shim_thread* thread, int sig, siginfo_t* info, bool need_interrupt) {
    ...
    /* only want to check if sighandler exists without actual invocation, so don't reset even if SA_RESETHAND */
    __rt_sighandler_t handler = __get_sighandler(thread, sig, /*allowreset=*/false);
